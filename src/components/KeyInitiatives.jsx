...
function InitiativeCard({ item, onClick }) {
  return (
    <button
      type="button"
...
function ProfileSection({ title, children, accent = 'indigo' }) {
  return (
...
function InitiativeDetail({ item, onBack }) {
  return (
    <div className="rounded-3xl border border-slate-200 bg-white shadow-[0_18px_45px_rgba(15,23,42,0.08)]" style={{ animation: 'fadeSlideIn 320ms ease' }}>
  ...
  return (
    <section className="w-full rounded-3xl bg-slate-100 px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className={`transition-all duration-300 ${selectedInitiative ? 'pointer-events-none max-h-0 overflow-hidden opacity-0 -translate-y-2' : 'opacity-100 translate-y-0'}`} aria-hidden={!!selectedInitiative}>
